...
export const instructorSignup = async (req, res, next) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ newInstructor, massage: "Ragistration successfully done" });
...
export const instructorupdate = async (req, res, next) => {
  ...
  try {
    if (req.file) {
      const updateDate = await instructorModel.findByIdAndUpdate(
        id,
        {
          $set: {
            ...req.body,
      ...
      res
        .status(200)
        .json({ updateDate, massage: "Instrutor deteils updated sucessfully" });
    } else {
      const updateDate = await instructorModel.findByIdAndUpdate(
        id,
        {
...
export const instructorForgetPassword = async (req, res, next) => {
  ...
  try {
    if (!password) {
      const findEmail = await instructorModel.findOne({
        email: req.body.resetEmail,
      });
      if (!findEmail) return next(error(404, "Your account does't exits"));
      res
        .status(200)
        .json({ id: findEmail._id, massage: "Email verification success" });
    } else {
      const hashPassword = await bcrypt.hash(password, 10);
      const resetPassword = await instructorModel.findByIdAndUpdate(
        id,
        {
          $set: {
            password: hashPassword,
      ...
      if (!resetPassword)
        return next(error, (404, "instructor deteils not found "));

      res
        .status(200)
        .json({ resetPassword, massage: "Password reset sucessfully" });
    }
  } catch (err) {
    next(err);
  }
...
export const incrementPresent = async (req, res, next) => {
  const id = req.query.id;
  try {
    const totalPresent = await instructorModel.findByIdAndUpdate(
      id,
      { $inc: { present: 1 } },
      { new: true }
    );
    ...
    res
      ...
      .json({
        totalPresent,
        massage: "Instructor attendense add successfully",
      });
  } catch (err) {
    next(err);
  }
